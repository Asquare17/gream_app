...
class _GreamDetailsPageState extends State<GreamDetailsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        builder: (context, AsyncSnapshot<DocumentSnapshot> snapshot) {
          ...
          if(snapshot.hasData && snapshot.data.data != null) {
            ...
            FirestoreCloudDb().getUserDoc(getUser.uID).get().then((value) {
              ...
              _isLiked = FirestoreCloudDb().isLiked(documentID: widget.documentReference.documentID, listOfLikes: mainUserModel.likedGreamPosts);
                      ...
                      child: Column(
                        ...
                        children: [
                          ListTile(
                            ...
                            subtitle: Text(Jiffy.unix(greamitPost.postTimestamp).fromNow()),
                            trailing: FlatButton.icon(
                                ...
                                icon: Icon(
                                  FirestoreCloudDb().isFollowing(userID: greamitPost.postUserID, listOfFollowers: mainUserModel.following) ? Icons.check : Icons.add,
                                  color: appTheme.primaryColor,
                                  size: 24.0,
                                ),
                                label: Text(
                                  FirestoreCloudDb().isFollowing(userID: greamitPost.postUserID, listOfFollowers: mainUserModel.following)  ? 'FOLLOWING' : 'FOLLOW',
                                  style: TextStyle(
                                      color: appTheme.primaryColor, fontSize: 18.0),
                                )
                            ),
                          ),
                          Text(
                          ...
                          Padding(
                            ...
                            child: Text(
                              ...
                              style:
                              TextStyle(height: 1.5, fontStyle: FontStyle.italic),
                            ),
                          ),
                          Container(
                            height: 40,
                            child: ListView.builder(
                                ...
                                itemBuilder: (BuildContext context, int index) {
                                  return CustomTag(widget.greamitPost.categoryList[index]);
                                }),
                          ),
                          Padding(
                            ...
                            child: Container(
                              ...
                              child: FutureBuilder<WebMetaData>(
                                  future: getWebsiteMetadata(websiteString: widget.greamitPost.link),
                          ...
                          Row(
                            children: [
                              Text(
                                greamitPost.likes.length.toString() + ' Likes',
                                style: TextStyle(color: Colors.black45),
                              ),
                              SizedBox(width: 15.0),
                              Text(
                                greamitPost.comments.length.toString() + ' Comments',
                                style: TextStyle(color: Colors.black45),
                              ),
                            ],
                          ),
                      ...
                      Expanded(
                        child: ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              return Container(
                                  height: 100,
                                  child: _buildCommentSection(comment: greamitPost.comments[index]));
                            }
                        ),
                      ),
                  ...
                  child: Column(
                    ...
                    children: [
                      ListTile(
                        ...
                        subtitle: Text(Jiffy.unix(widget.greamitPost.postTimestamp).fromNow()),
                        trailing: greamitPost.postUserID != getUser.uID ? FlatButton.icon(
                            onPressed: () {
                              setState(() {
                                _isFollowing = !_isFollowing;
                              });
                      ...
                      Row(
                        children: [
                          Text(
                            widget.greamitPost.likes.length.toString() + ' Likes',
                            style: TextStyle(color: Colors.black45),
                          ),
                          SizedBox(width: 15.0),
                          Text(
                            widget.greamitPost.comments.length.toString() + ' Comments',
                            style: TextStyle(color: Colors.black45),
                          ),
                          // Expanded(
                          //   child: Text(
                  ...
                  Container(
                    ...
                    child: ListView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          return Container(
                              height: 100,
                              child: _buildCommentSection(comment: widget.greamitPost.comments[index]));
                        }
                    ),
                  ),
      ...
      bottomNavigationBar: _commentClicked ? ListTile(
        leading: CircleAvatar(),
        title: TextField(
        ...
        trailing: IconButton(
          ...
          onPressed: () {
            ...
            FirestoreCloudDb().addComment(commentData: {
              ...
              "userID" : getUser.uID,
            }, greamDocumentReference: widget.documentReference).then((value) {
                ...
                FlatButton.icon(
                    onPressed: () {
                      ...
                      FirestoreCloudDb().likeUpdate(
                          ...
                          profileDocRef: FirestoreCloudDb().getUserDoc(getUser.uID),
                          likeList: greamitPost.likes
                      );

                    },
                    icon: _isLiked ? Icon(Icons.favorite, color: Colors.red,) : Icon(Icons.favorite_border, color: Colors.black54,),
                    label: Text(
                      'Like',
